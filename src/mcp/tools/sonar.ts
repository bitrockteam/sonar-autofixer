...
export const getQualityGateStatus = async (
	...
	const baseUrl = sonarBaseUrl
		? normalizeBaseUrl(sonarBaseUrl)
		: "https://sonarcloud.io";
	...
	try {
		...
		const statusData =
			(await statusResponse.json()) as QualityGateProjectStatus;
		...
		throw new Error(
			`Failed to fetch SonarQube quality gate status: ${errorMessage}`,
		);
	}
};
